...
import type { Subject, Admission } from '@/types';
...
export default function AbcAnalysis({ admissions, subjects, currencySymbol }: AbcAnalysisProps) {
 ...
 const productsInReceipt = new Set(receipt.items.map(i => i.subjectId));
 productsInReceipt.forEach(pid => {
 // Pre-initialize with receipt item data if available
 if (!productDataMap[pid]) {
 const item = receipt.items.find(i => i.subjectId === pid);
 productDataMap[pid] = { revenue: 0, quantity: 0, orderCount: 0, name: item?.name || 'Unknown Subject' };
 }
 productDataMap[pid].orderCount++;
 });
 receipt.items.forEach(item => {
 if (!productDataMap[item.subjectId]) {
 productDataMap[item.subjectId] = { revenue: 0, quantity: 0, orderCount: 0, name: item.name };
 }
 productDataMap[item.subjectId].revenue += item.price * item.quantity;
 productDataMap[item.subjectId].quantity += item.quantity;
 });
 });

 const totalBookingValue = Object.values(productDataMap).reduce((sum, { revenue }) => sum + revenue, 0);

 if (totalBookingValue === 0) {
 ...
 const cumulativePercent = (cumulativeRevenue / totalBookingValue) * 100;
 ...
 }, [admissions, subjects, searchQuery]);
 ...
 <CardDescription>
 Categorizes subjects based on their revenue contribution.
 </CardDescription>
 </div>
 ...
 <CategoryTable subjects={all} currencySymbol={currencySymbol} onRowClick={setSelectedProduct} />
 </TabsContent>
 <TabsContent value="classA">
 <CategoryTable subjects={classA} currencySymbol={currencySymbol} onRowClick={setSelectedProduct} />
 </TabsContent>
 <TabsContent value="classB">
 <CategoryTable subjects={classB} currencySymbol={currencySymbol} onRowClick={setSelectedProduct} />
 </TabsContent>
 <TabsContent value="classC">
 <CategoryTable subjects={classC} currencySymbol={currencySymbol} onRowClick={setSelectedProduct} />
 </TabsContent>
 </Tabs>
 ) : (
 ...
 <span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Subject Insight</span>
 </div>
 ...
 <div className="bg-muted/40 p-4 rounded-md border border-primary/5">
...
function CategoryTable({ subjects, currencySymbol, onRowClick }: { subjects: ProductAnalysis[], currencySymbol: string, onRowClick: (p: ProductAnalysis) => void }) {
 if (subjects.length === 0) {
 return <div className="text-center text-muted-foreground py-10">No subjects found.</div>
 }
 return (
