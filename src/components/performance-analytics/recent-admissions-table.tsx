...
export default function RecentSalesTable({ admissions, currencySymbol }: RecentSalesTableProps) {
 return (
 <Card>
 <CardHeader>
 ...
 <TableBody>
 {admissions.length > 0 ? (
 admissions.slice(0, 20).map((receipt) => (
 <TableRow key={receipt.id}>
 <TableCell>
 <div className="font-medium">{receipt.customer?.name || 'Walk-in Student'}</div>
 </TableCell>
