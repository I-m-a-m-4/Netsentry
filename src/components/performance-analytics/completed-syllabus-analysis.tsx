...
import { Subject, Admission } from '@/types';
...
export default function DeadStockAnalysis({ subjects, admissions, currencySymbol }: DeadStockAnalysisProps) {
 ...
 r.items.forEach(item => soldProductIds.add(item.subjectId));
 }
 });

 // Filter subjects that are in stock but haven't sold in 60 days
 return subjects
 ...
 <div className="pt-2">
 <Link href="/syllabus-tracker?sortBy=stock-desc" className="text-xs text-primary flex items-center gap-1 hover:underline">
