...
export default function PaymentMethodDistribution({ admissions, currencySymbol }: PaymentMethodDistributionProps) {
 ...
 const totalBookingValue = data.reduce((sum, item) => sum + item.value, 0);
 ...
 <div className={cn("p-1.5 rounded-md bg-background border")} style={{ borderColor: COLORS[index % COLORS.length] }}>
 {getIcon(item.name)}
 </div>
 <div>
 ...
 {totalBookingValue > 0 ? ((item.value / totalBookingValue) * 100).toFixed(1) : 0}%
 </p>
 </div>
 </div>
 ))}
