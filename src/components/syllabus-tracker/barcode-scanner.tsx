...
export function BarcodeScanner({ onScan, isOpen, onClose }: BarcodeScannerProps) {
 ...
 <DialogHeader className="p-4 bg-zinc-900/60 absolute top-0 left-0 right-0 z-50 flex flex-row items-center justify-between text-white border-b border-white/10 space-y-0">
 <DialogTitle className="text-lg font-medium">Scan Barcode</DialogTitle>
 <Button variant="ghost" size="icon" onClick={handleClose} className="text-white hover:bg-white/10 rounded-md h-8 w-8">
 <X className="h-5 w-5" />
 </Button>
 </DialogHeader>
 ...
 size="icon"
 onClick={toggleTorch}
 className="bg-zinc-900/80 border-white/10 text-white rounded-md h-14 w-14 hover:bg-zinc-800"
 >
 ...
 <Button onClick={handleTryAgain} variant="secondary" className="rounded-md">
 Try Again
 </Button>
 </motion.div>
 )}
