...
import { useBusiness, useAcademy } from '@/context/academy-context';
...
type ParsedProduct = Partial<Omit<Subject, 'id' | 'academyId' | 'imageHint' | 'lowStockThreshold'>>;

// Mappings for common CSV header variations
const HEADER_MAPPINGS: { [key: string]: string[] } = {
 name: ['Name', 'Subject Name', 'Item Name', 'Title'],
 sku: ['SKU', 'Code', 'Item Code'],
 // Prioritize more specific category names by listing them first
 category: ['Category', 'Categories', 'Subject Category', 'Type'],
...
export default function ImportDialog({ isOpen, onOpenChange, onSuccess, academyId, subjects }: ImportDialogProps) {
 const { toast } = useToast();
 const firestore = useFirestore();
 const academy = useBusiness();
 const { triggerConfetti, triggerRefresh } = useAcademy();
 ...
 name: row[mappedHeaders.name!] || 'Untitled Subject',
 ...
 if (!academyId || !firestore || parsedData.length === 0 || !academy || !subjects) {
 toast({
 variant: 'destructive',
 title: 'Import Failed',
 description: 'Cannot import subjects without required session data.',
 });
 return;
 }
 ...
 const currentProductCount = subjects.length;

 if (limit !== Infinity && currentProductCount + parsedData.length > limit) {
 toast({
 variant: 'destructive',
 title: 'Subject Limit Exceeded',
 description: `This import of ${parsedData.length} subjects would exceed your plan's limit of ${limit}. You currently have ${currentProductCount} subjects.`,
 duration: 8000,
 });
 return;
 }
 ...
 try {
 const businessDocRef = doc(firestore, 'businessInstances', academyId);
 ...
 description: `${parsedData.length} subjects have been added to your inventory.`,
 });
 onSuccess();
 triggerRefresh();
 } catch (e) {
 ...
 variant: 'destructive',
 title: 'Import Failed',
 description: 'An error occurred while saving the subjects. Please try again.',
 });
 } finally {
 ...
 <p>Upload a CSV file to bulk-add subjects. Your file must have columns for at least <strong>Name</strong> and <strong>Price</strong>. We also support common variations like 'Regular Price' (from WooCommerce) and various 'Image' headers.</p>
 ...
 {isParsing ? 'Parsing...' : `${parsedData.length} subjects found.`}
 </p>
 </div>
 ...
 <Button size="lg" onClick={handleImport} disabled={isParsing || isImporting || parsedData.length === 0 || !!error || !academyId}>
