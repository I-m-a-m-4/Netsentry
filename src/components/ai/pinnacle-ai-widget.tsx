...
export function PinnacleAIWidget() {
 ...
 >
 <Card className={`flex overflow-hidden border-primary/20 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 ${isExpanded ? 'w-[90vw] md:w-[800px] h-[80vh] flex-row' : 'w-[350px] sm:w-[400px] h-[500px] flex-col'}` }>
 ...
 <CardHeader className=" from-primary/10 to-primary/5 border-b pb-3 pt-4 px-4 flex flex-row justify-between items-center space-y-0 shrink-0">
 ...
 Engine of Success 
 <span className={`px-1.5 py-0.5 rounded-md text-[10px] font-bold ${aiCredits > 0 ? 'bg-primary/20 text-primary' : 'bg-red-500/20 text-red-500'}`}>
 {aiCredits} Credits
 </span>
 </p>
 </div>
 ...
 <Button variant="ghost" size="icon" className="h-8 w-8 rounded-md" onClick={() => setIsExpanded(!isExpanded)}>
 {isExpanded ? <Minimize2 size={16} /> : <Maximize2 size={16} />}
 </Button>
 <Button variant="ghost" size="icon" className="h-8 w-8 rounded-md" onClick={() => setIsOpen(false)}>
 <X size={16} />
 </Button>
 </div>
 </CardHeader>
 ...
 <div className="rounded-lg px-4 py-3 text-sm bg-muted/60 border rounded-tl-sm flex items-center gap-1.5 ">
 <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 1 }} className="w-1.5 h-1.5 bg-primary/60 rounded-md" />
 <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 1, delay: 0.2 }} className="w-1.5 h-1.5 bg-primary/60 rounded-md" />
 <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 1, delay: 0.4 }} className="w-1.5 h-1.5 bg-primary/60 rounded-md" />
 </div>
 </div>
 )}
 </div>
 ...
 {/* Paywall Overlay */}
 {aiCredits <= 0 && (
 <div className="absolute inset-0 bg-background/80 flex flex-col items-center justify-center p-6 text-center z-10">
 <div className="bg-card p-6 rounded-lg border max-w-sm">
 <div className="mx-auto w-12 h-12 bg-red-100 text-red-500 rounded-md flex items-center justify-center mb-4">
 <Lock size={24} />
 </div>
 ...
 className="pr-10 rounded-md bg-muted/50 border-muted focus-visible:ring-primary/50"
 />
 <Button
 type="submit"
 size="icon"
 disabled={!input.trim() || isLoading || aiCredits <= 0}
 className="absolute right-1 w-8 h-8 rounded-md"
 >
 ...
 className="w-14 h-14 rounded-md bg-primary text-primary-foreground flex items-center justify-center border-2 border-primary-foreground/20 hover: transition-shadow relative overflow-hidden group"
 >
