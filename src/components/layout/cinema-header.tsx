...
export default function CinemaHeader({ threshold = 50 }: { threshold?: number }) {
 ...
 scrolled ? "h-16 bg-white border-b border-slate-200 " : "h-24 bg-transparent"
 )}
 >
 ...
 className={cn(
 "w-12 h-12 flex items-center justify-center rounded-md transition-all duration-300 border border-slate-200",
 ...
 className="fixed top-0 right-0 bottom-0 z-[100] w-full max-w-sm bg-white flex flex-col"
 >
 {/* Header of Drawer */}
 ...
 <button 
 onClick={() => setIsMenuOpen(false)}
 className="w-10 h-10 flex items-center justify-center rounded-md bg-slate-100 text-slate-950 hover:bg-slate-200 transition-colors"
 >
 <X className="w-5 h-5" />
 </button>
 </div>
 ...
 <div className="flex flex-col gap-0 border-2 border-dashed border-slate-200 rounded-md overflow-hidden">
 ...
 onClick={() => setIsMenuOpen(false)}
 className="w-full text-center hover:bg-[#0f172a] transition-colors text-xs font-bold text-white tracking-[0.2em] font-dm-sans bg-[#1e293b] rounded-md py-4 uppercase"
 >
 Dashboard
 </Link>
 <button 
