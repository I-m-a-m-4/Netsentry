...
interface ImportCustomersDialogProps {
 ...
 onSuccess: () => void;
 academyId: string;
 existingCustomers: Student[];
}

type ParsedCustomer = Partial<Pick<Student, 'name' | 'email' | 'phone' | 'code'>>;
type ParsedCustomerWithEmail = ParsedCustomer & { email: string; };

const HEADER_MAPPINGS: { [key: string]: string[] } = {
 name: ['name', 'Name', 'Full Name', 'Student Name'],
 email: ['email', 'Email', 'Email Address'],
 phone: ['phone', 'Phone', 'Phone Number', 'Mobile'],
 code: ['code', 'Code', 'Unique Code', 'Student Code', 'ID'],
};

export default function ImportCustomersDialog({ isOpen, onOpenChange, onSuccess, academyId, existingCustomers }: ImportCustomersDialogProps) {
 ...
 description: `${processedData.length - newData.length} students already exist and will be skipped.`
 })
 }
 ...
 if (!academyId || !firestore || parsedData.length === 0) return;
 setIsImporting(true);
 try {
 const customersRef = collection(firestore, 'students');
 const statsRef = doc(firestore, 'businessInstances', academyId, 'stats', 'overall');
 ...
 description: `${parsedData.length} new students have been added.`,
 });
 onSuccess();
 } catch (e) {
 ...
 variant: 'destructive',
 title: 'Import Failed',
 description: 'An error occurred while saving the students. Please try again.',
 });
 } finally {
 ...
 <DialogDescription>
 Upload a CSV file to bulk-add students. Ensure your file has columns for 'Name' and 'Email' or 'Phone'.
 </DialogDescription>
 </DialogHeader>
 ...
 {isParsing ? 'Parsing...' : `${parsedData.length} new students found.`}
 </p>
 </div>
 ...
 {parsedData.length > 0 && (
 <div className='mt-4'>
 <h4 className='text-sm font-medium mb-2'>Preview of new students to import:</h4>
