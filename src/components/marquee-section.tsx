...
export function MarqueeSection() {
 return (
 <div className="pd_press_section py-6 border-b border-slate-100 bg-white/50 ">
 ...
 <div key={`m1-${index}`} className="flex items-center gap-3 px-4 py-2 rounded-md border border-slate-100 bg-white/80 ">
 <span className="flex-shrink-0 p-1 bg-slate-50 rounded-md">
 {item.icon}
 </span>
 ...
 {MARQUEE_ITEMS.map((item, index) => (
 <div key={`m2-${index}`} className="flex items-center gap-3 px-4 py-2 rounded-md border border-slate-100 bg-white/80 ">
 <span className="flex-shrink-0 p-1 bg-slate-50 rounded-md">
 {item.icon}
 </span>
 <span className="font-medium text-slate-600 whitespace-nowrap text-sm">{item.label}</span>
